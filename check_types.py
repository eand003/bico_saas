...
for table in ['clientes', 'maquinas']:
    req_url = f"{url}/rest/v1/{table}?user_id=eq.not-a-uuid&limit=1"
    req = urllib.request.Request(req_url, headers=headers)
    try:
        with urllib.request.urlopen(req) as r:
            print(f"Table '{table}' query succeeded (user_id is likely TEXT or compatible): status {r.status}")
    except urllib.error.HTTPError as e:
        print(f"Table '{table}' query failed: status {e.code}")
